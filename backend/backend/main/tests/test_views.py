...
class VideoApi(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.video0 = Video.objects.create(
    ...
    def test_create_video(self):
        ...
        temp_file = File(open(video_path, "rb"))
        uploaded_file = SimpleUploadedFile(
            "test_video.mp4", temp_file.read(), content_type="multipart/form-data"
        )
    ...
    def test_create_comment(self):
        ...
        api_client.force_authenticate(user=self.user)
        data = {"text": "test comment", "video": self.video0.id}
    ...
    def test_video_comments(self):
        ...
        url = (
            reverse("main:video_comments", kwargs={"pk": self.video0.id})
            + "?limit=10&offset=0"
        )
        response = api_client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
